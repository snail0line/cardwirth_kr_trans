...
### 말투(tone) 변형 — 두 구조, 둘 다 한 묶음

같은 대사를 말투(口調)별로 갈아 보여주는 구조가 **두 가지**다. 둘 다 그룹으로 묶어 표시한다.

**구조 1 — `<Talk type="Dialog">` 안 여러 `<Dialog>`** (쿠폰 분기)
- `<RequiredCoupons>` 값(`＿「尊大」` → 尊大)을 말투 라벨로 매핑(거만/거침/노인/공손/여성/남성…) — `tone_of`.
- 같은 `<Talk>` 안 `<Dialog>` 들 = 말투 변형 → **`id(Talk)`** 기준으로 묶음.

**구조 2 — `<Branch type="MultiStep" step="…口調"> <Contents>` 아래 `name="0..N"` 별도 `<Talk type="Message">` 들**
- 口調 스텝 값(0..N)마다 다른 Talk 로 갈라 놓은 형태. 각 Talk 가 한 말투.
- 조상 Branch 가 `type="MultiStep"` 이고 `step` 에 `口調` 가 있으면 → **`id(Branch)`** 기준으로 묶음(Text 한정).

공통: 멤버 1개뿐인 그룹은 해제. 프런트(`web/app.js`)는 **연속된 같은 group** 을 한 묶음으로 렌더.
구현: `extract.py`(그룹키), `context.py tone_of`(라벨). ※ 말투 설정용 화면 등 한 `<Talk type="Dialog">`
안에 여러 대사가 섞인 특수 메뉴는 그 Talk 통째로 한 묶음이 된다(의도된 동작).
